...
from losses import TripletMSELoss, SupConLoss, HiDistanceXentLoss
...
wandb.init(project="CLPR_singleALLRE_sampler_hidiswandb_v3")

# load parsers
parsers = {}
for lang in dfg_function:
    LANGUAGE = Language('/home/chen/workspace/codeproject/CL4acrossVersionSC/model_OneVulCrossVersion/ourCLPT/ourSampler_ourLoss/parser/my-languages.so', lang)
    parser = Parser()
...
# 设置日志
logging.config.fileConfig('/home/chen/workspace/codeproject/CL4acrossVersionSC/model_OneVulCrossVersion/ourCLPT/ourSampler_ourLoss/logging_CLPTALLVersionSampler_wandb.cfg')
...
class RuntimeContext(object):
    ...
    def get_config_file_name(self):
        ...
        config_file = "/home/chen/workspace/codeproject/CL4acrossVersionSC/model_OneVulCrossVersion/ourCLPT/ourSampler_ourLoss/devCLPT_ALLRESampler_hidis_wandb.cfg"
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        with open('/home/chen/workspace/codeproject/CL4acrossVersionSC/dataset/data_OneVulCrossVersion/datawithVersionInfo/contracts_withVersion_04-08.jsonl') as f:
            for line in f:
                line = line.strip()
                js = json.loads(line)
...
def train(args, train_dataset, model, tokenizer):
    ...
    HiDistanceXent = HiDistanceXentLoss().to(args.device)

    # loss_SupCon_list = []
    output_dir_tsne = '/home/chen/workspace/codeproject/CL4acrossVersionSC/model_OneVulCrossVersion/ourCLPT/t-SNE_images_crossVersionSampler_HiDis-v3'  # 指定保存图像的输出目录
    xent_lambda = 1.0
    margin_value = 10
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            with autocast():
                ...
                # weight_batch = torch.ones_like(labels)  # 假设y_batch是你的标签张量

                loss_SupCon  = HiDistanceXent(xent_lambda, encoder_embedding, labels, versions, margin= margin_value)
                wandb.log({"loss_SupCon_hidis": loss_SupCon})
                #loss_SupCon 是个tensor，只需要把其中的值取出来

                # loss_SupCon_numpy = loss_SupCon.detach().data.item()
                # loss_SupCon_list.append(loss_SupCon_numpy)
                # print('loss_SupCon_list is: ', loss_SupCon_list)
                ...
                # print('proportion is: ', proportion)
                # print('proportion*loss_SupCon',proportion*loss_SupCon)
            # loss = loss_CrossEntropy + proportion*loss_SupCon
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1_macro'] > best_f1:
                        best_f1 = results['eval_f1_macro']
                        logger.info("  "+"*"*20)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    #oringinal eval_dataloader using sequential sampler
    ...
    with torch.no_grad():
        ...
        with autocast():
            ...
            # loss_SupCon = supcon_loss(encoder_embedding, labels)
            weight_batch = torch.ones_like(labels)  # 假设y_batch是你的标签张量
            loss_SupCon  = HiDistanceXent(xent_lambda, encoder_embedding, labels, versions, margin= margin_value, weight = weight_batch)
            ...
            # 记录当前批次的损失
            # 立即记录当前批次的损失
            wandb.log({"eval_loss": eval_batch_loss.item(), "batch_idx": batch_idx})
    ...
    result = {
        ...
        "eval_FPR": float(FPR),
        "eval_MCC": float(mcc),
        "avg_eval_loss": float(eval_loss.item() / nb_eval_steps)
    }
...
def main():
    ...
    #RobertaTokenizer加载graphcodebert-base的词表
