...
class HiDistanceLoss(nn.Module):
    ...
    def forward(self, features, labels = None, versions=None, margin = 10.0,
        ...
        # xy_1 = x.mm(y.t()) *2
        # xy_2 = x @ y.t() *2
        # distance_matrix_1 = x_norm_squared + y_norm_squared - x_y_transpose
        
        pdist = nn.PairwiseDistance(p=2)
        distance_matrix_1 = pdist(x,y)
        
        distance_matrix_all = x_norm * x_norm + y_norm * y_norm - 2 * x.mm(y.T)
        distance_matrix_squared_clamped = torch.clamp(distance_matrix_all, min=0)
        distance_matrix = torch.sqrt(distance_matrix_squared_clamped)
