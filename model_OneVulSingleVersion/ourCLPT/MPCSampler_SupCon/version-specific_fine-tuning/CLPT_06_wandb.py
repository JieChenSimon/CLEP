...
wandb.init(project="CLPR_singleALLRE_3")

# load parsers
parsers = {}
for lang in dfg_function:
    LANGUAGE = Language('/home/chen/workspace/codeproject/CL4acrossVersionSC/modelSingleVulSingleVersion/parser/my-languages.so', lang)
    parser = Parser()
...
# 设置日志
logging.config.fileConfig('/home/chen/workspace/codeproject/CL4acrossVersionSC/modelSingleVulSingleVersion/logging_CPTLearningALLRE_wandb.cfg')
...
class RuntimeContext(object):
    ...
    def get_config_file_name(self):
        ...
        config_file = "/home/chen/workspace/codeproject/CL4acrossVersionSC/modelSingleVulSingleVersion/devCLPre_ALLRE_wandb.cfg"
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        with torch.no_grad():
            lm_loss_CrossEntropy, logit, encoder_embedding = model(inputs_ids_1, position_idx_1, attn_mask_1, labels)
            proportion = args.alpha * lm_loss_CrossEntropy.detach().data.item() / (lm_loss_CrossEntropy.detach().data.item() + loss_SupCon.detach().data.item())
            loss_SupCon = supcon_loss(encoder_embedding.unsqueeze(1), labels)
        
            eval_loss_CrossEntropy = lm_loss_CrossEntropy.mean().item()
            eval_loss_SupCon = loss_SupCon.item()

            eval_loss += eval_loss_CrossEntropy + proportion * eval_loss_SupCon
            logits.append(logit.cpu().numpy())
            y_trues.append(labels.cpu().numpy())
        nb_eval_steps += 1
    ...
    with open(os.path.join(args.output_dir, "predictions.txt"), 'w') as f:
        for example, pred in zip(test_dataloader.examples, y_preds):
            if pred:
